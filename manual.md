...
# Welcome to **MaNo**, the **Ma**rkdown **No**tepad
...
It allows you to find the native behavior you're already very familiar with.
...
This small tutorial will teach you how to write Markdown with the MaNo editor.
...
We selected the word "Computers" and made it a link with `Ctrl`+`L`.
...
> Hitting `F9` will automatically change the reference (#mushrooms) to
> (#mushrooms-and-co). Hit `F9` now to see how it acts on the link below:
...
Finally, note that MaNo can choose smartly between code block and inline
block depending if your selection contains line feeds or not.
Try to highlight "myVariable" below and hit `Ctrl`+`Alt`+`C` (Code Block):

To activate debug mode, set myVariable to `true`.

You see it is the same as pressing `Ctrl`+`Alt`+`D` (Inline Block) because
...
(Inline Block), you'll see it's the same as `Ctrl`+`Alt`+`C` (Code Block),
...
The only way you can insert a code block or an inline code is when you don't
have any selection (empty control) then you can fill it with what you want.
...
Title1 | Title2
-------|-------
ContentCell_1 | ContentCell_2
ContentCell_3 | ContentCell_4
ContentCell_5 | ContentCell_6
...
[My Webpage](http://mougino.free.fr)
...
feedback at [mougino@free.fr](mailto:mougino@free.fr)
